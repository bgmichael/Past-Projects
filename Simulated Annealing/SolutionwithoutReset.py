...
def generateRandomSuccessor(vector):
    ...
    return successor
def Simulated_Annealing_Linear(problem, schedule):
    vector = gen_solution(problem)
    current = generateRandomSuccessor(vector)
    next = generateRandomSuccessor(current)
    current = mapVectorToSolution(current, problem)
    next = mapVectorToSolution(next, problem)

    delta = next - current

def ScheduleCheck(T, sublist):
