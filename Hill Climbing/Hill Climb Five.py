...
def generate_random_problem(n):
    ...
    problem = []
    for i in range(n):
        num = random.randint(-20000, 20000)
        while num == 0:
            num = random.randint(-20000, 20000)
        problem.append(num)
    return problem
...
def generate_random_subset(problem_list):
    ...
    length2 = random.randint(2, (int(problem_length**.7)))
...
def main():
    ...
    # BruteForceClosestPair_no_square(i)
    # endTime = time.process_time()
    # timevalue = (format(endTime - startTime, "6.4f"))
    # timelist_closestpair.append(timevalue)

    for n in range(200, 205):
        startTime = time.process_time()
        print()
        print("Problem length: ", n)
        problem1 = generate_random_problem(n)
        guaranteeASolution(problem1)
        # print()
        print("The problem list after guarenteeASolution: ", problem1)
        ...
        while not foundSolution and not noImprovementPossible:
            if currentScore == 0:
                ...
                bestScore = 0
                print("Found zero initially: ", 0)
            else:
                # pick the best of x neighbors
                howManyNeighbors = 100000  # how many neighbors do you want to try?
                bestSuccessor = None
                ...
                for i in range(1):
                    i = i + 1
                    if bestScore == 0:
                        print("Score is equal to Zero: ", bestScore)
                        break
                    # print()
