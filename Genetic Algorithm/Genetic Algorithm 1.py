...
def Genetic_Algorithm(n):
    ...
    #while sum == 0 not met:
        #select the fitter individuals for reproduction#
        #recombine between those individuals#
        #mutate those individuals#
        #evaluate the fitness of the modified individuals#
        #generate a new population, generation#
    ...
    problem_list1 = guaranteeASolution(problem_list)
    random_solution = gen_solution(problem_list1)
    test_population = Produce_Initial_Population(10, problem_list)
...
def Selection(population, fitness_modifier):
    ...
    fitness_tracker = sorted(fitness_tracker)


    # iterator_bound = fittest_individual_fittness * fitness_modifier
    # Modifier_Bound = fitness_tracker[0] * fitness_modifier
    # while fitne

    #########################################################
    # median_sample_one_fitness = sum(population[random.randint(2, len(population) - 1)])
    # median_sample_two_fitness = sum(population[random.randint(2, len(population) - 1)])
    # artifiical_median_value = abs((abs(fittest_individual_fittness) + abs(least_fit_individual_fittness) + abs(median_sample_one_fitness)\
    #                            + abs(median_sample_two_fitness))/4)
    # percent_to_median = artifiical_median_value / fittest_individual_fittness
    # acceptable_fittness = fittest_individual_fittness * percent_to_median
...
def main():
    ...
    problem_list = generate_random_problem(14)
    pop = Produce_Initial_Population(10, problem_list)
    selected_pop = Selection(pop, .50)
    print(selected_pop)
